...
class SGFormer(nn.Module):
    def __init__(self, adj_channels, in_channels, hidden_channels, out_channels,
                 trans_num_layers=1, trans_num_heads=1, alpha=0.5, trans_dropout=0.5, trans_use_bn=True, trans_use_residual=True, trans_use_weight=True, trans_use_act=True,
        ...
        super().__init__()
        self.trans_conv=TransConv(in_channels,hidden_channels,trans_num_layers,trans_num_heads,alpha,trans_dropout,trans_use_bn,trans_use_residual,trans_use_weight)
    ...
    def forward(self, X, A):
        ...
        for b in range(batch):
            x1=self.trans_conv(X[b], A[b].to_sparse().indices())
            if self.use_graph:
                x2=self.graph_conv(X[b], A[b])

...
class GConv(nn.Module):
    def __init__(self, in_dim, hid_dim, out_dim, dropout):
        ...
        self.gc1 = GConvLayer(in_dim, hid_dim) 
        self.gc2 = GConvLayer(hid_dim, out_dim) 
...
class GTAD(nn.Module):
    def __init__(self, adj_dim, in_dim, hid_dim, out_dim, dropout):
        ...
        for i in range(in_dim):
            ...
            self.conv_layer_per_feat_.append(GConv(1, hid_dim, hid_dim, dropout))
        ...
        self.classifier = CLF(adj_dim, in_dim, hid_dim, out_dim, dropout)

        c = copy.deepcopy
        attn = MHSA(in_dim, in_dim * hid_dim)
    ...
    def forward(self, x, eigenvalue, eigenvector):
        HKs = []
        emb_per_feat = []
        for i in range(self.n_feats):
            HKs.append(self.compute_hk(eigenvalue, eigenvector, self.scale_per_feat[i].to(x.device)))

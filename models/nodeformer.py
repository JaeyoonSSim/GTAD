...
def denominator_gumbel(qs, ks, device):
    all_ones = torch.ones([ks.shape[0]]).to(device)
...
def kernelized_softmax(device, query, key, value, kernel_transformation, projection_matrix=None, edge_index=None, tau=0.25, return_weight=True):
    '''
    ...
    z_den = denominator(query_prime, key_prime, device)
    ...
    if return_weight: # query edge prob for computing edge-level reg loss, this step requires O(E)
        ...
        attn_normalizer = denominator(query_prime, key_prime, device) # [N, B, H]
...
def kernelized_gumbel_softmax(device, query, key, value, kernel_transformation, projection_matrix=None, edge_index=None,
    ...
    z_den = denominator_gumbel(query_prime, key_t_gumbel, device) # [N, B, H, K]
...
class NodeFormerConv(nn.Module):
    ...
    '''
    def __init__(self, device, in_channels, out_channels, num_heads, kernel_transformation=softmax_kernel_transformation, projection_matrix_type='a',
    ...
    def forward(self, z, adjs, tau):
        ...
        if self.use_gumbel and self.training:  # only using Gumbel noise for training
            z_next, weight = kernelized_gumbel_softmax(self.device, query,key,value,self.kernel_transformation,projection_matrix, adjs.to_sparse().indices(),
                                                  self.nb_gumbel_sample, tau, self.use_edge_loss)
        else:
            z_next, weight = kernelized_softmax(self.device, query, key, value, self.kernel_transformation, projection_matrix, adjs.to_sparse().indices(),
        ...
        if self.use_edge_loss: # compute edge regularization loss on input adjacency
            row, col = adjs.to_sparse().indices()
...
class NodeFormer(nn.Module):
    ...
    '''
    def __init__(self, adj_channels, in_channels, hidden_channels, out_channels, device, num_layers=2, num_heads=4, dropout=0.0,
        ...
        for i in range(num_layers):
            self.convs.append(
                NodeFormerConv(device, hidden_channels, hidden_channels, num_heads=num_heads, kernel_transformation=kernel_transformation,
        ...
        self.linear = torch.nn.Linear(adj_channels * hidden_channels, adj_channels * hidden_channels // 2) # Readout
        self.linear2 = torch.nn.Linear(adj_channels * hidden_channels // 2, out_channels) # Predictor
    ...
    def forward(self, X, adjs, tau=1.0):
        ...
        for b in range(batch):
            ...
            for i, conv in enumerate(self.convs):
                if self.use_edge_loss:
                    z, link_loss = conv(z, adjs[b], tau)
                    link_loss_.append(link_loss)
                else:
                    z = conv(z, adjs[b], tau)
                if self.use_residual:
                    z += layer_[i].squeeze(2)
                if self.use_bn:
                    z = self.bns[i+1](z)
                if self.use_act:
                    z = self.activation(z)

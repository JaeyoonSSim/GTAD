...
def gcn_conv(x, adj, edge_weight):
    # row, col = edge_index
    ...
    for i in range(x.shape[1]):
        gcn_conv_output.append(adj @ x[:, i])  # [N, D]
    ...
    #     pdb.set_trace()
    #     gcn_conv_output.append(adj @ x[:,:, i])  # [N, D]
    # gcn_conv_output = torch.stack(gcn_conv_output, dim=2) # [N, H, D]
    # return gcn_conv_output
...
class DIFFormer(nn.Module):
    ...
    '''
    def __init__(self, adj_channels, in_channels, hidden_channels, out_channels, num_layers=2, num_heads=1, kernel='simple',
